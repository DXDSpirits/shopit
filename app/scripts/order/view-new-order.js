(function() {
    ...
    var sizeView = new (OrderInputView.extend({
        ...
        filterSizes: function() {
            ...
            var $circles = _.map(sizes, function(size) {
                return $('<span class="circle available"></span>').attr('data-size', size).text(size);
            }, this);
            this.$('.circles').html($circles);
        },
    ...
    App.Pages.NewOrder = new (App.PageView.extend({
        ...
        render: function() {
            ...
            this.$('#order-input-size').trigger('expand');
        }
    }))({el: $('#view-new-order')});
